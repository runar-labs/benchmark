...
use services_common::{Profile, Account, ServiceRecord, AggregatedData};
...
async fn main() {
    ...
    for profile in &profiles {
        ...
        client.publish("service_b/request", QoS::AtLeastOnce, false, payload.clone()).await.unwrap();
    ...
    for _ in 0..PROFILE_COUNT {
        if let Ok(Some((email, accounts))) = timeout(Duration::from_millis(TIMEOUT_MS), b_rx.recv()).await {
            b_results.insert(email, accounts);
        }
        if let Ok(Some((email, services))) = timeout(Duration::from_millis(TIMEOUT_MS), c_rx.recv()).await {
